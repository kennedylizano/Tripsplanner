...
class UserManager(models.Manager):
    def userValidor(self,postDATA):
        ...
        if len(postDATA["username"])< 2:
            errors["username"]= 'User name is requiered !!'
...
class TripManager(models.Manager):
    def tripValitor(self,postDATA):
        ...
        if len(postDATA["Destination"])< 5:
            errors["Destination"]= ' Destination shloud be more than 5 character !!'
            print(errors)
        if len(postDATA["description"])< 5:
            errors["description"]= ' Description shloud be more than 5 character!!'
            print(errors)
        if len(postDATA["startdate"])< 1:
            errors["startdate"]= ' Date is require !!'
            print(errors)
        if len(postDATA["enddate"])< 1:
            errors["enddate"]= ' EndDate is require !!'
            print(errors)
        if postDATA["startdate"] > postDATA['enddate']:
            errors["startdate"]="please choose a diferent date "
            print(errors)
